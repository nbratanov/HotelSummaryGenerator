...
def get_most_used_phrases():
    ...
    freq_table = get_word_count_in_all_documents(freq_matrix)
    phrases_map = {}
    print(freq_table['breakfast'])
    for review in reviews:
        words = word_tokenize(review)
        bigrams = list(nltk.ngrams(words, 2))
        for entry in bigrams:
            ...
            if tagged_bigram[0][1] == 'JJ' and (tagged_bigram[1][1] == 'NN' or tagged_bigram[1][1] == 'NNS'):
                try:
                    phrase_score = freq_table[entry[0]] + freq_table[entry[1]]
                    phrases_map[entry] = phrase_score
                except KeyError:
                    print(entry[0])
                    print(entry[1])

    sorted_phrases = dict(sorted(phrases_map.items(), key=lambda item: item[1], reverse=True))
